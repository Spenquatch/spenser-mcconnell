...
export const metadata = {
  title: 'Subscribe - DevSpace',
  description: 'Page description',
}

export default async function Subscribe() {
  let subscribeContent = null;

  try {
    subscribeContent = await strapiAPI.getSubscribeContent();
  } catch (error) {
    console.error('Failed to fetch subscribe content:', error);
          ...
          <div className="space-y-10">
            <section>
              ...
              <h1 className="h1 font-aspekta mb-5">
                {subscribeContent?.page_title || 'Never miss an update ✨'}
              </h1>
              {/* Page content */}
              <div className="text-slate-500 dark:text-slate-400 space-y-8">
                <p className="text-lg">
                  {subscribeContent?.page_description || 'This newsletter is written by Mark Ivings, who previously worked at Google, Medium, Vimeo, and Qonto. Here is what to expect by subscribing:'}
                </p>
                {subscribeContent?.newsletter_benefits && subscribeContent.newsletter_benefits.length > 0 ? (
                  <ul className="space-y-4">
                    {subscribeContent.newsletter_benefits.map((benefit, index) => (
